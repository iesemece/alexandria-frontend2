...
public class MiBibliotecaController {
    @FXML
    private FlowPane listaBiblioteca;
        ...
        call.enqueue(new Callback<List<Libro>>() {
            ...
            public void onResponse(Call<List<Libro>> call, Response<List<Libro>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    Platform.runLater(() -> {
                        listaBiblioteca.getChildren().clear();
                        ...
                        for (Libro libro : response.body()) {
                            ...
                            javafx.scene.control.MenuItem eliminarItem = new javafx.scene.control.MenuItem("Eliminar de la biblioteca");
        ...
        apiService.eliminarLectura("Bearer " + token, libro.getId()).enqueue(new Callback<>() {
            ...
            public void onResponse(Call<Void> call, Response<Void> response) {
                if (response.isSuccessful()) {
                    Platform.runLater(() -> {
                        // Eliminar visualmente del FlowPane
                        listaBiblioteca.getChildren().removeIf(node -> {
