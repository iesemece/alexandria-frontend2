...
public class LecturasCompartidasController {
    ...
    @FXML
    private ListView<Libro> listViewLibrosCompartidos;
    ...
    private void initialize() {
        ...
        listViewLibrosCompartidos.setOnMouseClicked(event -> {
            if (event.getClickCount() == 2) {
                Libro libroSeleccionado = listViewLibrosCompartidos.getSelectionModel().getSelectedItem();
    ...
    private void mostrarLibrosCompartidosCon(UsuarioListado usuario) {
        ...
        ApiService apiService = ApiClient.getApiService();
        Call<List<Libro>> call = apiService.obtenerLibrosCompartidos(
                usuarioActualId,
                usuarioDestinoId,
                "Bearer " + token
        );
